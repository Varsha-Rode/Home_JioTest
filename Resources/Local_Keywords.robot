...
Check UI of Most Popular and Most Value section
    #Verifying Whether Most Popular and Most Value section UI and Functionality
    Check Most Popular and Most Value section
...
Checking Product Name
    ...
    ${value} =    Get Text    (//div[contains(@class, "product-container")]//div[@class="pop-prod-title"])[${i}]
...
Checking Product Specs
    ...
    #Reading Value from Excel
    ${value1} =    Reading Data of Shops Page from Excel    ${i+18}    2    ${sheet}
...
Checking Product Price
    ...
    ${value}=    Remove String    ${value}    ₹
    #Reading Value from Excel
    ${value1} =    Reading Data of Shops Page from Excel    ${i+18}    3    ${sheet}
    
    Should Be Equal As Strings    ${value}    ${value1}

Checking Redirection on Product
    #Verify smartphone url from Excel
    ${Vurl} =    Reading Data of Shops Page from Excel    ${i+18}    4    ${sheet}
...
Checking "View Details" UI
    ...
    #Wait For And Verify Content    //div[@id='bank-HSBCEMI-emi-table']//div[@class='emiThreshold-wrapper']    The minimum purchase amount needed to qualify for EMI is ₹2000 
    Wait For And Verify Element    //div[@id='bank-10-emi-table']
    Wait For And Click Element    //button[@class="close"]

Pincode Verification
    Scroll To    Eligible for delivery?
...
Stock Unavailable For Pincode
    ...
    Run Keyword and Ignore Error    Notify Me Button UI Verification     #possibly Site Issue
    Element Should Be Disabled    //button[@id="AddToCartBTN"]

Stock available For Pincode
    #Verify UI when a Product is Available
...
Confirm redirection of the breadcrumbs hyperlinks
    ...
    Scroll To                   //div[@class='j-breadcrumbs__item j-breadcrumbs__firstItem']
    Wait For And Click Element    //div[@class='j-breadcrumbs__item j-breadcrumbs__firstItem']
    URL Validation    ${URL}
    Go Back
...
Checking "View All Specifications"
    Wait For And Verify Content    //div[@class="description-button-block"]/span    View all specifications
    Scroll To    //div[@class="description-button-block"]
    Wait For And Click Element    //div[@class="description-button-block"]
    Wait For And Verify Element    //a[@id="tab-specification-link"]
...
Notify Me Button UI Verification
    ...
    Wait For And Click Element    //button[@id="notifyyMe"]
    ...
    Wait For And Verify Element    //input[@id='notifyEmail']
    Wait For And Verify Element    //input[@id='notifymobilenumber']
    Wait For And Verify Element    //button[@onclick="document.getElementById('notifypopup').style.display='none'"]
    Wait For And Verify Element    //button[@id='notifySubmit']
    Wait For And Verify Element    //div[@onclick="document.getElementById('notifypopup').style.display='none'"]//*[name()='svg']
    
    #Checking Notify Me Popup Functionality
    Input Text    //input[@id='notifyEmail']    123@gmail.com
    Input Text    //input[@id='notifymobilenumber']    1234567890

    Wait For And Click Element    //button[@id='notifySubmit']
    Wait For And Verify Content    //div[@class='success-message-popup']    You will be notified when the product will be available for delivery.
    Wait For And Click Element    //button[@id='notifyClose']
   
Confirm the functionality of Quantity's '+' and '-'
    Scroll To                    //button[@class='qty-increment']
    
    #Checking Decrement Button Disabled
    Wait For And Verify Element    //button[@class='qty-decrement btnDisabled']

    #Increasing Product Quantity
    Wait For And Click Element    //button[@class='qty-increment']

    #Checking Whether Product has reached its Quantity Limit
    ${q_limit}=    Run Keyword And Return Status    Wait For And Verify Element    //div[@class='max-limit-rchd']
    IF    ${q_limit}
        #Checking For Error Message
        Wait For And Verify Content    //div[@class='max-limit-rchd']    Maximum limit reached!
        
        #Checking Whether Increment button is disabled 
        Wait For And Verify Element    //button[@class='qty-increment btnDisabled']

        Checking "Add To Cart" UI
        Redirection On Clicking Shop  //div[@id="digi-cartpopup-GotoCart"]    https://www.jio.com/shop/cart    Shopping_cart
    ELSE
        #Verfying Product Quantity Increased
        Wait For And Verify Content    //div[@class='current-quantity-text']    2

        #Decreasing Product Quantity
        Wait For And Click Element    //button[@class='qty-decrement']
        ...
        Checking "Add To Cart" UI
        Wait For And Verify Element    //div[@id="digi-cartpopup-ContinueShopping"]//a[@href="/shop/c/smartphones"]
        Redirection On Clicking Shop  //div[@id="digi-cartpopup-GotoCart"]    https://www.jio.com/shop/cart    Shopping_cart
...
Checking "Shipping & Delivery"
    ...
    #Verify Content of Accordion
    Wait For And Verify Content    //div[@id="tab-shipping"]//strong    Standard Delivery
...
Checking Filter UI
    ...
    FOR  ${i}  IN RANGE    1    ${titles+1}
        ...
        ELSE
            ...
            #Run Keyword And Ignore Error    Wait For And Click Element    (//div[@class="filter-section"])[${i}]//div[@class="show-more-less show-more"]
...
Click on Bin Button
    ...
    Wait For And Click Element    //td[@class="cart-bundle-remove"]/a
    Wait For And Verify Content    //div[@class="information_message positive"]/p    Product has been removed from your cart.    
    Wait For And Verify Content    //div[contains(@class,'jio-empty-cart')]/div    Your shopping cart is empty.
...
UI Validation of Delivery Address Page
    # Validating Content 
    Wait For And Verify Content    //div[@class="title_holder"]/h2    ${Delivery_Data[${0}]}

    FOR  ${i}  IN RANGE    1    4
        #Validating Address lines
        Element Attribute Value Should Be    (//div[@class="formwidth"]/dd/input)[${i}]    placeholder    ${Delivery_Data[${i}]}   
    END
    
    #Validating Pincode and Country
    Wait For And Verify Element    (//div[@class="formwidth"]/dd/input)[4]        
    ${Fetching_Pincode}=    Execute Javascript    return document.getElementById("address.postcode").value
    Log to Console    ${Fetching_Pincode}
    # Convert To Integer    ${Fetching_Pincode}    
...
Payment Method Page Functionality
    ...
    FOR  ${i}  IN RANGE  1    ${Count+1}
        ...
        IF    ${Terms}== True
            ...
            Scroll Element Into View    xpath=(//span[@class="digi-terms"])[${i}]
...
# -------------------------------
Redirection On Clicking for Home
   [Arguments]       ${Button}          ${WebPageURL}    ${Sc_name}
    Scroll To                           ${Button}                 
    Wait For And Click Element          ${Button}              
    Sleep     1s
...
Pop-Up Verification Home
    ...
    IF  "${Title}" == "Quick Recharge"
        ...
        #Quick Recharge sub-title
            ...
            #Back button
            Click Element                      xpath=//div[@class="j-modal-container"]//button[@data-popup-close="modal-info"]
...
Need Guidance Section Validation                                            #akshaansh.h
   ...
   ${VB}  Set Variable   0
   WHILE  ${VB} < 4
      Redirection On Clicking     xpath=//div[@class='j-button-group btn_fullwidth_icon']//button[@aria-label='${Button_Name}[${VB}]']   ${Guidance_UrlList}[${VB}]    ${Button_Name}[${VB}]  
      ${VB}=   Evaluate     ${VB} + 1
   END
...
Infographics Verificationn
    ...
    ${length_subtitle}=    Get Length       ${elements_subtitle}
   
    #Text Verification For Subtitle
    FOR  ${subtitle}  IN  @{elements_subtitle}
                ${vari}=          Get Text        ${subtitle}
                #Log To Console        ${vari}
    ...
    #Verifying Title is Bold and Subtext is of Lighter Font
    ...
    #Images are in alternate positions
    FOR  ${it}  IN RANGE    1   ${length_title} + 1
        ${val}=     Set Variable        ${2}
        ${zero}=    Set Variable        ${0}
        ${modulo_result}    Evaluate    ${it}%${val}
        #Log To Console    ${modulo_result}
        
        IF     ${modulo_result} == ${zero}
            #${val}=    Get Element Attribute        xpath://*[@id="__next"]/div[2]/main/section/section/section[3]/div[${it}]/div[1]  
            IF  "${App_name}" == "JioMart"
                    CSS Verification      xpath://*[@id="__next"]/div[2]/main/section/section/section[3]/div[${it}]/div[1]        order        1
            ELSE
                CSS Verification      xpath://*[@id="__next"]/div[2]/main/section/section/section[3]/div[${it}]/div[1]        order        2
            END
        ELSE
            IF  "${App_name}" == "JioMart"
                    CSS Verification      xpath://*[@id="__next"]/div[2]/main/section/section/section[3]/div[${it}]/div[1]        order        2
        ...
        END
    END
    ${elementsy}=     Set Variable  0

MyJio App Validation
    Reading Data of Apps Page from Excel    2
    Wait For And Click Element            xpath://div[@aria-label="MyJio"]//button[2]
    sleep    1s                             
    URL Validation      ${App_URL}
    # Log To Console      ${App_URL}
    Infographics Verificationn

JioSavan App Validation
    Reading Data of Apps Page from Excel    3 
    Wait For And Click Element            xpath://div[@aria-label="JioSaavn"]//button[2]
    sleep    1s 
    URL Validation      ${App_URL}
   # Log To Console      ${App_URL}
    Infographics Verificationn
    Checking Infographics Link

JioTV App Validation
    Reading Data of Apps Page from Excel    4
    Wait For And Click Element            xpath://div[@aria-label="JioTV"]//button[2]
    sleep    1s 
    URL Validation      ${App_URL}
    #Log To Console      ${App_URL}
    Infographics Verificationn

JioCall App Validation

    Reading Data of Apps Page from Excel    12
    Wait For And Click Element            xpath://div[@aria-label="JioCall"]//button[2]
    sleep    1s 
    URL Validation      ${App_URL}
    #Log To Console      ${App_URL}
    Infographics Verificationn
...
JioPOS Lite App Validation
    Reading Data of Apps Page from Excel    7
    Wait For And Click Element            xpath://div[@aria-label="JioPOS Lite"]//button[2]
    Sleep        1s
    URL Validation      ${App_URL}
    #Log To Console      ${App_URL}
    Infographics Verificationn

JioHealth Hub App Validation
    Reading Data of Apps Page from Excel    8
    Wait For And Click Element            xpath://div[@aria-label="JioHealthHub"]//button[2]
    URL Validation      ${App_URL}
    #Log To Console      ${App_URL}
    Infographics Verificationn

JioChat App Validation
    Reading Data of Apps Page from Excel    11
    Wait For And Click Element            xpath://div[@aria-label="JioChat"]//button[2]
    Sleep        1s
    URL Validation      ${App_URL}
    #Log To Console      ${App_URL}
    Infographics Verificationn
...
JioMeet App Validation
    Reading Data of Apps Page from Excel    9
    Wait For And Click Element            xpath://div[@aria-label="JioMeet"]//button[2]
    Sleep        1s
    URL Validation      ${App_URL}
    #Log To Console      ${App_URL}
    Infographics Verificationn

JioSecurity App Validation
    Reading Data of Apps Page from Excel    15
    Wait For And Click Element            xpath://div[@aria-label="JioSecurity"]//button[2]
    Sleep        1s
    URL Validation      ${App_URL}
    #Log To Console      ${App_URL}
    Infographics Verificationn
    Checking Infographics Link
    Checking Infographics Button

JioNews App Validation
    Reading Data of Apps Page from Excel    10
    Wait For And Click Element            xpath://div[@aria-label="JioNews"]//button[2]
    Sleep        1s
    URL Validation      ${App_URL}
    #Log To Console      ${App_URL}
    Infographics Verificationn

JioMart App Validation
    Reading Data of Apps Page from Excel    6
    Wait For And Click Element            xpath://div[@aria-label="JioMart"]//button[2]
    Sleep        1s
    URL Validation      ${App_URL}
    #Log To Console      ${App_URL}
    Checking Infographics Caption
    Infographics Verificationn

JioCinema App Validation
    Reading Data of Apps Page from Excel    5
    Wait For And Click Element            xpath://div[@aria-label="JioCinema"]//button[2]
    Sleep        1s
    URL Validation      ${App_URL}
    #Log To Console      ${App_URL}
    Infographics Verificationn

JioGames App Validation
    Reading Data of Apps Page from Excel    13
    Wait For And Click Element            xpath://div[@aria-label="JioGames"]//button[2]
    Sleep        1s
    URL Validation      ${App_URL}
    #Log To Console      ${App_URL}
    Checking Infographics Caption
...
JioCloud App Validation
    Reading Data of Apps Page from Excel    14
    Wait For And Click Element            xpath://div[@aria-label="JioCloud"]//button[2]
    Sleep        1s
    URL Validation      ${App_URL}
    #Log To Console      ${App_URL}
    Infographics Verificationn

JioHome App Validation
    Reading Data of Apps Page from Excel    17
    Wait For And Click Element            xpath://div[@aria-label="JioHome"]//button[2]
    Sleep        1s
    URL Validation      ${App_URL}
    #Log To Console      ${App_URL}
    Infographics Verificationn

JioPages App Validation
    Reading Data of Apps Page from Excel    16
    Wait For And Click Element            xpath://div[@aria-label="JioPages"]//button[2]
    Sleep        1s
    URL Validation      ${App_URL}
    #Log To Console      ${App_URL}
    Infographics Verificationn
...
Checking Infographics Link
    ...
    #${Link_webelement}=        Get WebElements        xpath://section[@data-testid="app-features-section"]//div[@class="j-grid"]//div[@class="j-contentBlock__description"]//a  
...
Checking Infographics Caption
    ...
    ${length_caption}=    Get Length       ${elements_caption}
    # Log To Console    ${elements_caption}
    # Log To Console    ${length_caption}
    #Text Verification For Title
...
Checking Infographics Button
    ...
    ${length_button}=    Get Length       ${elements_button}
    # Log To Console    ${elements_button}
    # Log To Console    ${length_button}
    #Text Verification For Title
...
Infographics Verification Merged
    FOR  ${ROW}  IN RANGE    2     18
        Reading Data of Apps Page from Excel     ${ROW}
        Wait For And Click Element            xpath://div[@aria-label="${App_Name}"]//button[2]
        Sleep        1s
        URL Validation      ${App_URL}
        ${count}=     Set Variable  0
        ${elements_title}=  Get WebElements   xpath=//section[@data-testid="app-features-section"]//div[@class="j-grid"]//div[@class="j-contentBlock__title"]    
        ${length_title}=    Get Length       ${elements_title}
        # Log To Console    ${elements_title}
        # Log To Console    ${length_title}
        ...
        ${elements_subtitle}=  Get WebElements   xpath=//section[@data-testid="app-features-section"]//div[@class="j-grid"]//div[@class="j-contentBlock__description"]   
        ${length_subtitle}=    Get Length       ${elements_subtitle}
        # Log To Console    ${elements_subtitle}
        # Log To Console    ${length_subtitle}
        #Text Verification For Subtitle
        FOR  ${subtitle}  IN  @{elements_subtitle}
                ${vari}=          Get Text        ${subtitle}
                # Log To Console        ${vari}
        ...
        ${elementsy}=     Set Variable  0
        #Checking Links
        IF  "${App_name}" == "JioSaavan" or "${App_name}" == "JioChat" or "${App_name}" == "JioSecurity"
                Checking InfoGraphicsLinks  
        END
        #checking caption
        IF  "${App_name}" == "JioMart" or "JioGames"  
                Checking Infographics Caption
        END

        #checking button
        IF  "${App_name}" == "JioGames" or "JioSecurity" or "JioPages"  
                Checking Infographics Button
        END
        Go Back
    END

Redirection of other buttons
    [Arguments]    ${locator}
    Wait For And Click Element    ${locator}
...
Plan Card Verification-2
    ...
    # ${choice}  Dialogs.Get Value From User   message=You Want To Read Data?
...
View Details Pop up Verification 2
    ...
    Scroll Element Into View     ${View_Button} 
    Sleep    3s 
...
Plan Page UI Verification-2
    ...
    Wait For And Verify Element      (//div[contains(.,'Other Postpaid plans')])[6]
...
Verify Banner redirection
    ...
    Redirection On Clicking           xpath:(//div[@class='j-promo-card bg--primary img--filled orientation--horizontal no-children']//div[@class='j-promo-card__content-body'])[1]     ${Servies_redirect[0]}    Service_Banner_redirect   
...
Verify Get JioFiber button redirection
    ...
    Redirection On Clicking           xpath://button[@aria-label="Get JioFiber"]   ${Servies_redirect[0]}    Get_JioFiber_redirect
...
Verify Functionality of Video cards
    ...
    FOR  ${i}  IN RANGE    0    ${total_videos}
        ...
        ${title}=    Get Text                    xpath:(//div[@class="j-contentBlock__title"]/div[@class="j-heading j-text-body-m"])[${i+1}]

        # Scrolling Video cards into View
        Scroll Element Into View                 xpath:(//a[@class="j-link j-card__hidden-cta"]/div[@class="j-heading j-text-body-m"])[${i+1}]
        
        # Clicking on Video card
        Click Element                            xpath:(//a[@class="j-link j-card__hidden-cta"]/div[@class="j-heading j-text-body-m"])[${i+1}]

        # Verifying that the title of Video matches with title of Video card
        Wait For And Verify Content              xpath://div[@class="j-text j-text-body-m"]/h5[@class="j-heading j-text-heading-xs"]    ${title}

        # Waiting till the iframe(youtube window) is visible
        Wait Until Element Is Visible            xpath://iframe[contains(@src,'https://www.youtube.com/embed/')]    20s
        ...
        Wait Until Page Contains Element         xpath://div[contains(@class,'playing-mode')]    20s
...
Verify redirection of Best Selling JioFiber Accessories section
    ...
    FOR  ${i}  IN RANGE    0    3
        ...
        Redirection On Clicking                xpath://section[@aria-label="jioaccessories"]//div[@class="h-100"][${i+1}]    ${Servies_redirect[${i+1}]}    accessories${i+1}
    END

    # Checking redirection of View all accessories button
    Sleep    5s
    Redirection On Clicking                      xpath://button[@aria-label="View all accessories"]    ${Servies_redirect[4]}    Accessories_page
...
Verify Redirection of More from Jio Promo cards
    ...
    FOR  ${i}  IN RANGE    0    ${total_banners}
        ...
        Redirection On Clicking                  xpath://div[@class="j-promo-card__content-body"]//h4[@class="j-heading j-text-heading-s"]    ${Servies_redirect[0]}    More_from_jio_redirect
    END
